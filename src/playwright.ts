...
const processListing = async (page: playwright.Page) => {
  console.log("Processing listing...");

  const listingUrl = page.url();
  const price1 = await (await (await page.$("div[class='hdp__sc-1tsvzbc-1 jRNYtf ds-chip'] span[class='Text-c11n-8-53-2__sc-aiai24-0 iyRhoe']"))?.getProperty("innerText"))?.jsonValue();
  const price2 = await (await (await page.$("div[class='hdp__sc-1tsvzbc-1 ds-chip'] div[class='ds-home-details-chip'] div[class='ds-summary-row-container'] div[class='ds-summary-row-content'] div[class='ds-summary-row'] span[class='Text-c11n-8-53-2__sc-aiai24-0 hdp__sc-b5iact-0 haTioN ezoFMW'] span span"))?.getProperty("innerText"))?.jsonValue();
